...
export default function CheckoutFormMotor({
  ...
  const handleNumberChange = (name: string, delta: number) => {
    setFormData((prev) => ({
      ...prev,
      [name]: Math.max(0, Number(prev[name as keyof typeof prev]) + delta),
    }));
  };
  ...
  const total =
    totalSewa + formData.helm * helmPrice + formData.mantel * mantelPrice;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const bookingData: Booking = {
      ...
      jenis_sewa: undefined, // Motor tidak ada pilihan jenis sewa
      helm: formData.helm || undefined,
      mantel: formData.mantel || undefined,
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-lg border border-netral-200">
            ...
            <div className="grid grid-cols-2 gap-4 items-center">
              <div>
                <label className="block text-sm text-netral-900 mb-2">
                  Helm
                </label>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => handleNumberChange("helm", -1)}
                ...
                <p className="text-xs text-netral-500 mt-2">
                  *Helm 5000/pcs/hari
                </p>
              </div>
              <div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => handleNumberChange("mantel", -1)}
                    className="px-3 py-1 rounded border"
                  >
                    -
                  </button>
        ...
        <div className="bg-white p-6 rounded-lg border border-netral-200 sticky top-24">
          ...
          <div className="space-y-3 mb-4">
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-medium text-netral-900">
                {convertRupiah(formData.helm * 5000)}
              </span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-netral-600">
                Mantel (x{formData.mantel})
              </span>
              <span className="font-medium text-netral-900">
                {convertRupiah(formData.mantel * 5000)}
              </span>
            </div>
          </div>
